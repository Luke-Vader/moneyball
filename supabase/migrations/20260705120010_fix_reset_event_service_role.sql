-- ============================================================
-- FIX: reset_event_data() checked is_admin(), which resolves via
-- auth.uid() — but /api/admin/reset-event calls this RPC through
-- the service-role client (required for auth.admin.deleteUser),
-- which carries no user JWT, so auth.uid() is null and is_admin()
-- always failed. Confirmed via live testing. Same fix already
-- applied to recompute_club_record — allow service_role too.
-- ============================================================
